...
int main() {
    ...
    while (!openList.empty()) {
        ...
        if (current == goal) {
            ...
            cout << endl;
            cout << "⏱️  OpenMP A* Time: " << duration << " seconds" << endl;
            return 0;
        }
        ...
        for (int i = 0; i < graph[current].size(); ++i) {
            long long neighbor = graph[current][i].target;
            double edge_len = graph[current][i].length;

            if (visited.count(neighbor)) continue;

            double tentative_g = gScore[current] + edge_len;
